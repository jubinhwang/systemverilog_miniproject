...
### 📝 프로젝트 개요 (Overview)
본 프로젝트는 **하만교육 2기** 과정의 일환으로 진행되었습니다. FPGA의 물리적 버튼과 UART 통신 명령어를 통해 **10000 카운터**를 제어하는 시스템을 설계하고 검증하는 것을 목표로 합니다.

주요 과제는 Verilog HDL로 UART(FIFO 포함) 및 카운터 로직을 설계하고, **SystemVerilog** 기반의 체계적인 테스트벤치를 구축하여 설계의 기능적 정확성을 검증한 후 최종 리포트를 생성하는 것입니다.
...
* **`BD_D,L,R`**: FPGA 보드의 물리적 버튼 입력을 처리하는 모듈입니다.
* **`COUNTER`**: 0부터 9999까지 카운팅을 수행하는 핵심 모듈입니다.
...
2.  **⚙️ Vivado 프로젝트 생성 및 설정**
    ...
    * **프로젝트 생성**: 새 프로젝트를 시작하고, 타겟 보드로 **Basys3**를 정확히 선택합니다.
